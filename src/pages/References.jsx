...
export default function References() {
    ...
    const Card = ({ r }) => (
        <div title={r.name} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{
                background: '#fff', borderRadius: 12, border: '1px solid #e7ded0',
                height: 120, display: 'flex', alignItems: 'center', justifyContent: 'center',
                padding: 16, boxSizing: 'border-box', boxShadow: '0 6px 18px rgba(120,100,60,0.10)',
            }}>
                {r.logo_url
            ...
            <span style={{ color: '#1e293b', fontSize: '0.85rem', fontWeight: 500, textAlign: 'center' }}>{r.name}</span>
        </div>
    );
        ...
        <div style={{
            ...
            <div style={{ width: '100%', maxWidth: 1240, margin: '0 auto' }}>
                <div style={{ marginBottom: 24 }}>
                    ...
                    <h1 style={{ color: '#0f172a', fontSize: '1.9rem', margin: '12px 0 4px' }}>{t('references.title')}</h1>
                    <p style={{ color: '#64748b', margin: 0, fontSize: '0.95rem' }}>{t('references.subtitle')}</p>
                </div>

                {items === null ? (
                    <p style={{ color: '#64748b' }}>…</p>
                ) : items.length === 0 ? (
                    <p style={{ color: '#64748b' }}>{t('references.empty')}</p>
                ) : (
                    <div style={{
                        display: 'grid',
